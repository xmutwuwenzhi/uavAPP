...
//	  picture.format        = PIX_FMT_RGB565;
	  ...
	  picture.y_buf   = vp_os_malloc( VIDEO_WIDTH * VIDEO_HEIGHT  );//4
	  picture.cr_buf  = vp_os_malloc( VIDEO_WIDTH * VIDEO_HEIGHT / 4 );//1
	  picture.cb_buf  = vp_os_malloc( VIDEO_WIDTH * VIDEO_HEIGHT / 4);//1

	  picture.y_line_size   = VIDEO_WIDTH ;  //Ã¿ÐÐµÄÏñËØÊýÄ¿
	  picture.cb_line_size  = VIDEO_WIDTH / 2;
	  picture.cr_line_size  = VIDEO_WIDTH / 2;

	yuv2rgbconf.rgb_format = VP_STAGES_RGB_FORMAT_RGB24;
//	yuv2rgbconf.rgb_format =
...
#endif
/*
	my_stages[my_pipeline.nb_stages].type    = VP_API_FILTER_YUV2RGB;
	my_stages[my_pipeline.nb_stages].cfg     = (void*)&yuv2rgbconf;
	my_stages[my_pipeline.nb_stages].funcs   = vp_stages_yuv2rgb_funcs;
	my_pipeline.nb_stages++;
*/
